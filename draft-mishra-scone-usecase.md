...
Function (UPF) in 5G and the Packet Data Network Gateway (P-GW) in 4G can generate throughput advice
...
and additionally introduces terms  in this document are described in this section.

- 4G - Fourth Generation mobile network
The technology also known as Long-Term Evolution (LTE), defined by the 3rd Generation
...
Is a logical label used in 4G/LTE networks to identify
the external data network that a User Equipment (UE) requests to connect to. The APN specifies the
target data network (e.g., the Internet or a private network) and is used by the network core to
select the appropriate gateway (P-GW) and establish the corresponding data connection.
It is functionally equivalent to the Data Network Name (DNN) used in 5G systems.
...
A Network Element that serves as the access point for subscribers in wireline broadband networks.
It establishes and manages subscriber sessions, aggregates traffic from multiple subscriber access
nodes, and routes this traffic to the service provider's core network. BNG functions include
subscriber authentication, IP address assignment, policy enforcement, and quality of service management.
It typically supports subscriber session protocols such as DHCP, PPPoE, or IPoE, and interacts
...
Typical CPE includes routers, modems, or gateways that provide access and management for residential or enterprise
services.

- DHCP - Dynamic Host Configuration Protocol
A network management protocol used to dynamically assign IP addresses and other configuration parameters to devices
on a network, enabling automatic and centralized network configuration.

- DNN - Data Network Name
A Data Network Name (DNN) identifies the external data network that a User Equipment (UE) connects
to within a 5G system. The DNN specifies the target data network (for example, the Internet or a
private network) and is used by the 5G Core to establish and manage the corresponding PDU session.
It is functionally equivalent to the Access Point Name (APN) used in 4G/LTE systems.

- EPC - The Evolved Packet Core
Is the all-IP core architecture for 4G/LTE, responsible for managing user sessions, mobility,
and the integration of data and voice traffic over packet-switched networks.

- EPS Bearer - Evolved Packet System Bearer
In 4G LTE networks, an EPS bearer is a logical data pipe/path with specific Quality of Service (QoS) parameters that
carries user data between the User Equipment (UE) and the Packet Data Network Gateway (P-GW). The EPS bearer ensures
end-to-end delivery of IP packets within a mobile network with particular handling characteristics, such as
priority,latency, packet loss error rate and bit rate (guaranteed or best effort). There are two main types:
the Default EPS Bearer which provides always-on best-effort connectivity, and Dedicated
EPS Bearers configured for services with specialized/differentiated QoS requirements.

- eNB - evolved node B
Radio Access Network node connecting user equipment to the 4G core network.

- gNB - Next Generation Node B
Radio Access Network node connecting user equipment to the 5G core network.

- IPoE IP over Ethernet
A protocol that delivers IP packets directly over Ethernet without requiring a login or session establishment,
commonly used in broadband networks in conjunction with DHCP for IP address assignment.

- LTE - Long-Term Evolution
4G wireless broadband technology and related network architecture.
Is the set of 3GPP standards that define the 4G wireless broadband technology and
its evolved network architecture (Evolved Packet Core, or EPC).

- P-GW aka PDN-GW - Public Data Network Gateway
Is the network function within the Evolved Packet Core (EPC) that provides IP connectivity between the user
equipment and external packet data networks, such as the Internet. It is responsible for user-plane traffic
routing and applying policy decisions.

- PDU - Packet Data Unit
A PDU is a unit of information at a given protocol layer, such as an IP packet at the network layer.
Specifically in 5G, a PDU Session represents a logical connection that carries one or more PDUs between
the User Equipment (UE) and a Data Network (DN) through the User Plane Function (UPF). PDU Sessions support
multiple types of PDUs, including IPv4, IPv6, Ethernet frames, and unstructured data, and are associated
with one or more QoS Flows that define handling and quality requirements. The PDU framework is
essential for managing application data transport and quality of service within the 3GPP system architecture.

- PCF - Policy Congrol Function
Is a core element of the 5G network that provides a unified policy framework to govern network behavior.
It is responsible for creating, updating, and serving policy rules to other 5G control plane elements
to ensure consistent enforcement across the network.

- PPP - Point-to-Point Protocol
A data link layer communication protocol used to establish a direct connection between two nodes, commonly used
for dial-up and broadband internet connections to provide authentication, encryption, and compression.

- SMF - Session Management Function
Is a core element of the 5G network responsible for session management and IP address allocation.
It is specifically responsible for establishing, modifying, and releasing PDU Sessions,
managing QoS flows, and interacting with the UPF to ensure policy and traffic steering enforcement.

- UE - User Equipment
Is the term used for the subscriber's mobile device or endpoint (e.g., smartphone, tablet, etc)
that connects to the mobile network. The UE is responsible for all signaling and data procedures
necessary to establish and maintain communication with the network.

- UPF - User Plane Function
Is a core element of the 5G network that provides IP connectivity between the User Equipment (UE)
and external data networks (e.g., the Internet). It serves as the data anchor point for PDU
Sessions, enforcing traffic routing, packet inspection, and Quality of Service (QoS) policy decisions.
...
The 5G system comprises the 5G Radio Access Network (RAN) and
the 5G Packet Core. The 5G Packet Core is built on a cloud-native
Service-Based Architecture (SBA) and utilizes decoupled Network
Functions (NFs), which offers high flexibility for deploying
SCONE logic within the core.

The User Plane Function (UPF) serves as the on-path Network Element in the 5G core,
providing access to both subscriber policy and the user-plane connectivity between
the User Equipment (UE) (or client application endpoint) and the Internet.

The UPF is capable of generating SCONE throughput advice on a per-application-flow basis.
This functionality enables SCONE-aware endpoints to proactively adjust their sending rates.
...
relevant to SCONE manageability as SCONE packets traverse an established
PDU sessions:

1. PDU Session
   A logical connection between the UE and UPF (5G), allowing the UE to
   exchange IP packets with external networks such as the Internet or
   a private network.

3. IP Address Allocation
   During PDU Session establishment, the UE is allocated
   an IP address (IPv4, IPv6, or both) used for communication with an external network.
...
SCONE signaling operates exclusively over established PDU Sessions.
This ensures that SCONE Network Elements in the 5G network can unambiguously
associate advisory throughput advice with specific User Equipment (UEs) and
application flows. Since each PDU Session is bound to a unique Data Network Name
(DNN) and an allocated IP address, SCONE packets are handled precisely without
affecting unrelated traffic.

### Per-Flow Signaling
Throughput advice is applied on an UDP 4-tuple basis, requiring SCONE-capable
Network Elements in the 5G network to maintain flow-specific context. This
approach ensures that SCONE-aware endpoints receive targeted throughput advice
without unintended impact on unrelated traffic flows.

### QoS Considerations
In 5G, Quality of Service (QoS) is enforced at the granularity of QoS Flows.
A single PDU Session can contain multiple QoS Flows. Mobile Operators may configure
distinct QoS parameters for SCONE-enabled flows. However, SCONE-enabled flows are
typically used for adaptive bit-rate applications and are expected to be treated
...
In these cases, a SCONE-capable Network element needs to send dynamic updates to applications
...
Mobile Operators can integrate SCONE signaling into existing operational and
management frameworks to enable critical functions such as monitoring, troubleshooting,
and fault isolation.

Key Metrics and Analytics:

- Advisory Rate: Rate at which SCONE advisory messages issued per session
- Throughput Correlation: The correlation between issued SCONE advisories and
  subsequent user-plane throughput changes.
- Signaling Errors: Identification of error conditions where SCONE signaling
  fails to reach the User Equipment (UE).
...
SCONE can be integrated at the Packet Data Network Gateway (P-GW) or the Serving
Gateway (S-GW). Unlike 5G, traffic granularity is bearer-based rather than per-flow.
...
The SCONE application end-points may remain within the Wi-Fi network for the duration of a session,
              ...
              Npcf |      +-----+       |Nudm
                   +------+ SMF +-------+
                          +--+--+      ___  __
                             | N4     (   )(  )
   +----+   +--------+    +--+--+    (         )    +----------+
   | UE |---| gNodeB |----| UPF |----( Internet )---| Content  |
   |    |   |        |    |     |    (         )    | Provider |
   +----+   +--------+ N3 +- ---+ N6  (        )    +----------+
                              | N9     (__(___)
                            +-+---+
...
## N9 Interface
This interface interconnects two or more UPFs used in a data path.
The interface uses the GTP-U protocol for user traffic tunneling including roaming.

Note: In the scenario of 2 or more UPFs in the data path, only one UPF that
has access to subscriber policy would send "throughput advice" to the
...
Packet Data Unit (PDU) session is a logical path between a UE and UPF to
...
gNB and GTP-U tunnel between gNB and UPF (N3 interface). Application traffic flows
with different QoS requirements get mapped to different QoS treatments based on
packet filters and QoS rules configured on the UPF and UE.

Below is an example of data flow to/from a UE to the UPF.

1. Uplink Data Flow
    - Apps that are hosted on UE that generate application packets for
   communication (e.g. web browsing, video streaming).
    - These packets are transmitted to the gNB over the air interface and get
   mapped to different QoS treatments based on packet filters and QoS rules provided to the UE
...
2. Downlink Data Flow
    ...
    - The UPF encapsulates incoming data using GTP-U and forwards it over the N3 interface to the gNB.
      It maps traffic flows with different QoS requirements to different QoS treatments based on packet filters
      and QoS rules configured by SMF.
    - The gNB forwards the packets to the UE over the air-interface.  UE-side modem stack then transparently passes
      the eapplication packets to the app hosted on the UE.

In summary, the UPF is responsible for packet routing and forwarding, packet
inspection and filtering, participating in subscriber and flow policy enforcement,
